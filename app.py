...
def recipe():
    ...
    if get_ingredient ==[]:
        ...
        recipe = requests.get('https://api.spoonacular.com/recipes/findByIngredients', params={'apiKey': API_KEY, 'ingredients':ingredient, 'number': 8})
        list_recipe = recipe.json()
        return render_template('/user/recipe.html', list_recipe=list_recipe)

@app.route('/user/recipe/<int:recipe_id>', methods=["GET", "POST"])
